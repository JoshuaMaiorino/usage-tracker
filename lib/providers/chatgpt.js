import { credentialPath, decodeClaims, readCredentials, replaceCredentials, selectCredential, serializeCredentials } from '../credentials.js';
...
export function createChatgptProvider({ homeDir, fetchImpl = globalThis.fetch, now = Date.now } = {}) {
  ...
  const get = (path, credential, timeoutMs) => {
    const headers = { Authorization: `Bearer ${credential.accessToken}`, Accept: 'application/json', 'OpenAI-Beta': 'codex-1' };
    // This claim selects the account; decoding it does not authenticate the JWT.
    const accountId = [credential.accountId, ...[credential.entry.id_token, credential.accessToken]
      .map(token => decodeClaims(token)['https://api.openai.com/auth']?.chatgpt_account_id)].find(tokenString);
    if (accountId) headers['ChatGPT-Account-Id'] = accountId;
    return requestJson(fetchImpl, `${BASE_URL}/${path}`, { headers }, { id: 'chatgpt', now, timeoutMs });
  };
  return coalesce(async () => {
    ...
    const usageReceivedAt = now();
    let resetCredits;
    if (typeof payload.rate_limit_reset_credits?.available_count === 'number' && payload.rate_limit_reset_credits.available_count > 0) {
      try { resetCredits = await get('rate-limit-reset-credits', credential, 8000); } catch { /* optional metadata cannot discard usage */ }
    }
    return normalizeChatgpt(payload, { plan: credential.plan, resetCredits, now: usageReceivedAt });
  });
}
