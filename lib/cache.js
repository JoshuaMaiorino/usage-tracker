...
export class UsageCache {
  constructor({ providers = {}, intervalMs = DEFAULT_INTERVAL_MS, now = Date.now, random = Math.random } = {}) {
    ...
    this.states = new Map(ORDER.map((id) => [id, {
      ...
      blockedUntil: 0, rateLimitFailures: 0, inFlight: null,
    }]));
    this.running = false;
    this.timer = null;
  }
  ...
  fetchProvider(id, state) {
    ...
    state.inFlight = Promise.resolve().then(() => this.providers.get(id)()).then((value) => {
      ...
      if (state.error.code === 'rate_limited') {
        ...
        const fallback = Math.min(MAX_BACKOFF_MS, this.intervalMs * 2 ** Math.min(10, state.rateLimitFailures - 1) * jitter);
        const delay = typeof retryAfter === 'number' && Number.isFinite(retryAfter) && retryAfter >= 0 && retryAfter <= 8.64e15 - this.now() ? retryAfter : fallback;
        state.blockedUntil = this.now() + Math.max(MIN_REFRESH_INTERVAL_MS, delay);
      } else {
        state.rateLimitFailures = 0;
        state.blockedUntil = 0;
      }
